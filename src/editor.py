"""
This module contains classes for creating a Tile Grid UI and managing the grid, tiles, and carts.
Classes:
- Cart: Represents a cart object with its position, direction, and order.
- TileGridUI: Manages the Tile Grid UI, including grid initialization, tile selection, cart placement, and saving/loading the grid.
Methods:
- __init__(self, master): Initializes the TileGridUI object.
- setup_ui(self): Sets up the user interface elements.
- initialize_grid(self): Initializes the grid with the specified width and height.
- update_canvas_size(self): Updates the size of the canvas based on the grid dimensions.
- draw_grid(self): Draws the grid, including tiles, destination marker, and carts.
- get_tile_image(self, tile_index, size): Retrieves the image of a tile with the specified index and size.
